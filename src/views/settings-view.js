import { getSettings, saveSettings, applySettings } from '../store';
export function renderSettingsView(container) {
  ...
  `;
    ...
    const fontRange = container.querySelector('#font-size-range');
    const fontVal = container.querySelector('#font-size-val');
    const lhBtns = container.querySelector('#lh-btns');
    ...
    lhBtns.addEventListener('click', (e) => {
        ...
        const updated = { ...getSettings(), lineHeight: lh };
        saveSettings(updated);
        applySettings(updated);
    });
}
