import { getBookmarks, deleteBookmark, updateBookmarkNote, } from '../store';
...
export function renderBookmarksView(container) {
  ...
  `;
    renderList(container.querySelector('#bm-list'));
}
...
function attachCardHandlers(list, rerender) {
    list.addEventListener('click', (e) => {
        ...
        if (action === 'open') {
            ...
            if (bm)
                navigate(`/read/${bm.entryId}?bm=${bm.id}`);
            return;
        }
        if (action === 'delete') {
            deleteBookmark(bmId);
        ...
        if (action === 'edit') {
            const editArea = card.querySelector(`#edit-${bmId}`);
        ...
        if (action === 'edit-cancel') {
            const editArea = card.querySelector(`[id^="edit-"]`);
        ...
        if (action === 'edit-save') {
            ...
            const bm = getBookmarks().find(b => b.id === bmId);
            if (bm) {
                const preview = card.querySelector('.bookmark-note-preview');
                if (preview) {
                    preview.textContent = bm.note;
                }
                else if (bm.note) {
                    card.querySelector('.bookmark-meta')?.insertAdjacentHTML('afterend', `<span class="bookmark-note-preview">${esc(bm.note)}</span>`);
                }
            }
            return;
        }
