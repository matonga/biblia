...
import { addBookmark, getBookmarks, getLastPosition, saveLastPosition, } from '../store';
import { navigate } from '../router';
export function renderReaderView(container, entryId, bookmarkId, signal) {
  ...
  `;
    ...
    const fab = container.querySelector('#bookmark-fab');
    const modal = container.querySelector('#bm-modal');
    const bmNote = container.querySelector('#bm-note');
    const bmCancel = container.querySelector('#bm-cancel');
    const bmSave = container.querySelector('#bm-save');
    ...
    if (prev)
        prevBtn.addEventListener('click', () => navigate(`/read/${prev.id}`), { signal });
    if (next)
        nextBtn.addEventListener('click', () => navigate(`/read/${next.id}`), { signal });
    ...
    setTimeout(() => {
        ...
        if (bookmarkId) {
            ...
            if (bm) {
                restoreScrollPercent(bm.scrollPercent);
                return;
            }
        }
        const last = getLastPosition();
        if (last?.entryId === entryId)
            restoreScrollPercent(last.scrollPercent);
    }, 80);
    ...
    window.addEventListener('scroll', () => {
        ...
        scrollTimer = window.setTimeout(() => {
            if (!signal.aborted)
                saveLastPosition(entryId, getScrollPercent());
        }, 400);
    }, { passive: true, signal });
}
...
function restoreScrollPercent(pct) {
    const { scrollHeight, clientHeight } = document.documentElement;
    const max = scrollHeight - clientHeight;
    document.documentElement.scrollTop = (pct / 100) * max;
}
function showToast(msg, inReader = false) {
