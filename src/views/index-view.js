import { getBibleData, normalizeStr } from '../data';
...
export function renderIndexView(container) {
  ...
  `;
    const input = container.querySelector('#toc-search');
    const body = container.querySelector('#toc-body');
    ...
    body.addEventListener('click', (e) => {
        ...
        if (!target || target.dataset.leaf === 'true')
            return;
...
function renderBody(body, children) {
...
function renderTree(body, children) {
...
function buildBook(book, children) {
    ...
    <details class="toc-book" data-book-id="${book.id}">
      ...
      <div class="toc-chapters">
        ${chapters.map(ch => `<a href="#/read/${ch.id}" class="toc-chapter">${escHtml(ch.title)}</a>`).join('')}
      </div>
    </details>
  `;
}
function renderSearch(body, children) {
    const { entries } = getBibleData();
    const needle = normalizeStr(searchTerm.trim());
    const results = [];
    for (const entry of entries.values()) {
        if (normalizeStr(entry.title).includes(needle))
            results.push(entry);
...
function buildResult(entry, children) {
    const { entries } = getBibleData();
...
function openInTree(bookId, rootId) {
    if (rootId !== undefined) {
        const rootDetails = document.querySelector(`.toc-root[data-root-id="${rootId}"]`);
        if (rootDetails)
            rootDetails.open = true;
    }
    const bookDetails = document.querySelector(`.toc-book[data-book-id="${bookId}"]`);
    if (bookDetails) {
